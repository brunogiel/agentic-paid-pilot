...
⚠ Gotcha real: cada vez que se agrega una fuente nueva en el frontend hay que sumarla a `NURTURE_SOURCES`, o esos leads entran como `New`. Pasó en una implementación real con las sources de un popup nuevo.
...
Schema sugerido de la DB (el que corrió en una implementación real): `Name` (title), `Stage` (select), `Email` (email), `Phone` (phone_number), `Industry` (select), `Annual revenue` (select), `Qualified` (select), `Source` (select), `Message`, `Variant`, `Language`, `UTM campaign`, `UTM content` (rich_text).
